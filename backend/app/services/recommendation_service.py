...
class RecommendationService:
    ...
    def generate_recommendations(
        ...
        if metrics and metrics.accuracy < 0.5:
            rec = self._create_or_get_recommendation(
                ...
                description=f"El estudiante tiene un rendimiento global bajo ({int(metrics.accuracy * 100)}%). Se recomienda asignar actividades de repaso general.",
            ...
            if rec: generated_recs.append(rec)
        ...
        for state in mastery_states:
            if state.status == "at_risk" or state.mastery_score < 0.5:
                ...
                mc = db.query(MicroConcept).get(state.microconcept_id)
                ...
                rec = self._create_or_get_recommendation(
                    ...
                    description=f"El estudiante muestra dificultades en '{mc_name}' (Dominio: {int(state.mastery_score * 100)}%).",
        ...
        if metrics and metrics.median_response_time_ms > 30000:
             rec = self._create_or_get_recommendation(
                ...
                description=f"El tiempo de respuesta es muy alto ({metrics.median_response_time_ms / 1000}s). Podría indicar distracciones, fatiga o falta de comprensión profunda.",
    ...
    def apply_tutor_decision(
        ...
        """
        recommendation = db.query(RecommendationInstance).get(decision_data.recommendation_id)
