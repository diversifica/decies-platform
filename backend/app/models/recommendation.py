...
class RecommendationInstance(Base):
    ...
    student_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), 
        ForeignKey("students.id", name="recommendation_instances_student_id_fkey", ondelete="CASCADE"),
        nullable=False
    )
    ...
    microconcept_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), 
        ForeignKey("microconcepts.id", name="recommendation_instances_microconcept_id_fkey", ondelete="CASCADE"),
        nullable=True
    )
    ...
    # Relationships
    evidence: Mapped[list["RecommendationEvidence"]] = relationship("RecommendationEvidence", back_populates="recommendation", cascade="all, delete-orphan")
    decision: Mapped["TutorDecision"] = relationship("TutorDecision", back_populates="recommendation", uselist=False, cascade="all, delete-orphan")
...
class RecommendationEvidence(Base):
    ...
    recommendation_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("recommendation_instances.id", name="recommendation_evidence_recommendation_id_fkey", ondelete="CASCADE"),
        nullable=False
    )
    
    evidence_type: Mapped[str] = mapped_column(String, nullable=False) # e.g., "metric_value", "history_pattern"
    ...
    recommendation: Mapped["RecommendationInstance"] = relationship("RecommendationInstance", back_populates="evidence")
...
class TutorDecision(Base):
    ...
    recommendation_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("recommendation_instances.id", name="tutor_decisions_recommendation_id_fkey", ondelete="CASCADE"),
        nullable=False,
        unique=True
    )
    ...
    decision: Mapped[str] = mapped_column(String, nullable=False) # "accepted", "rejected" - mirroring status mostly
    decision_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    notes: Mapped[str] = mapped_column(Text, nullable=True)
    
    recommendation: Mapped["RecommendationInstance"] = relationship("RecommendationInstance", back_populates="decision")
