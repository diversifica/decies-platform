...
from app.models.metric import MetricAggregate, MasteryState
...
def context(db_session):
    ...
    uid = uuid.uuid4()
    u_stu = User(id=uuid.uuid4(), email=f"s_{uid}@test.com", role_id=role_student.id, full_name="S", hashed_password="x")
    ...
    u_tut = User(id=uuid.uuid4(), email=f"t_{uid}@test.com", role_id=role_tutor.id, full_name="T", hashed_password="x")
    ...
    # Academic Year
    year = AcademicYear(id=uuid.uuid4(), name=f"2024_{uid}", start_date=datetime.now(), end_date=datetime.now())
    db_session.add(year)
    
    # Subject/Term
    subject = Subject(id=uuid.uuid4(), name=f"Sub {uid}")
    db_session.add(subject)
    term = Term(id=uuid.uuid4(), name="Term 1", code="T1", start_date=datetime.now(), end_date=datetime.now(), academic_year_id=year.id)
    db_session.add(term)
